...
def build():
    ...
    story += [
        ...
        P("The argument is a directory of g1.txt ... g9.txt as written by the "
          ...
          "have to stay put."),
        P("It prints the table and then opens a window with four panels: the "
          "locus and the three circles, the two fitting domains on G(f), the "
          "radial residual of all overtones, and the decomposition of section 3 "
          "- the figures of this document, drawn from the data just measured "
          "rather than reproduced from it. --overtone N chooses the overtone of "
          "the top row, --save DIR writes a PNG instead of opening a window, "
          "--no-plot gives the table alone. Under QT_QPA_PLATFORM=offscreen the "
          "window is skipped automatically, so the same command line still "
          "works headless."),
