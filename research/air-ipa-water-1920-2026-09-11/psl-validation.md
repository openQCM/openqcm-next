# Validation of the phase-shifted Lorentzian on G — block A, on the data of 2026-09-11 (partial)

*Plan of 2026-09-15, block A: what can be verified offline on the nine dumps of 2026-09-11 before any bench
work. `scripts/psl_validation.py` with `scripts/psl_lib.py` (the chain, the rotated Lorentzian and its fit, shared
with `phase_shifted_lorentzian.py`). The claim under test, Marco's: the phase-shifted Lorentzian fitted to G
alone is the best estimator of f_s and Γ we have, and its case rests first on the comparison with the argmax
+ half-height estimator on the same sweeps, and only second on Kanazawa–Gordon. Block B (second sensor,
second board, third liquid, a live parallel column, a direct measurement of φ) is not started.*

## Summary of block A

| step | question | answer, in numbers |
|---|---|---|
| A1 | is the argmax bias the one the model predicts, f_argmax − f_res = Γ·tan(φ/2)? | on 45 sweeps: measured − predicted = +1 ± 28 Hz (sd), max 77 Hz; in units of Γ, mean +0.016, max 0.12 (air n = 1–5, where the fold plateau sits under the peak) |
| A2 | does the smoothing of the chain (SG 51 + spline) make the result? | raw samples against the chain: f_res −5…+3 Hz, Γ −29…+1 Hz, φ ±0.2°, over 15 cases |
| A3 | does the phase offset δ make it? | ±5° on the phase: f_res ≤ 21 Hz, Γ ≤ 70 Hz (n = 1) and ≤ 37 Hz (n ≥ 3); φ absorbs up to 4.3° of the 5° in liquid |
| A4 | with a known truth, which estimator is right? | synthetic sweeps through the divider and the detector: the fit returns f_s within 4 Hz, Γ within 5 Hz, φ within 0.3°; the argmax is off by Γ·tan(φ/2) (−331 against −340 predicted, water-like n = 5) and the half-height Γ is low by 3–6 % |
| A5 | is φ the same on the day before? | not done: the copy of the 2026-09-10 air dump was lost with the scratchpad |
| A6 | which uncertainty to quote? | the fit's covariance says 0.1–0.8 Hz on f_res; the three replicas scatter by 0.3–29 Hz: the replica scatter is the number, the covariance is not |
| A7 | is the model choice fragile? | a linear background changes f_res by ≤ 19 Hz in liquid but shifts φ by 6–12° in air (φ and background are degenerate on ±3Γ there); limiting the window where the sweep is clipped changes f_res by ≤ 15 Hz and Γ by ≤ 23 Hz |
| A8 | what is φ? | −8 → −27° from n = 1 to 9, the same in air and liquid within 1–4° except n = 1 (−8° air, −5° liquid); a pure board delay of 0.76–1.10 ns would give 1.4–2° at 5 MHz and 12–18° at 45 MHz: same order at the top, too small at the bottom |

What block A establishes: the difference between the two estimators is systematic, of the size and sign the
rotated Lorentzian predicts, independent of the smoothing and of the offset, and reproduced on synthetic data
where the truth is known and the fit recovers it while the argmax does not. What it does not establish: that
the result holds on another sensor or board, what φ is physically, and how the estimator behaves over a
plateau of a hundred sweeps.
...
![A1](figures/psl_validation/A1_bias_formula.png)

*Left: measured bias of argmax against the closed-form prediction, one point per sweep. Right: the residual of the formula in units of Γ against Γ.*
...
## A5 — the air dump of 2026-09-10

**Not done.** The only copy of that dump outside the repo was in the session scratchpad, which was cleared overnight; `sweep_data/` had been overwritten by the run of 2026-09-11. If Marco has another copy, the check is one command (`psl_validation.py` takes the folder as its second argument). Lesson recorded: raw data copied out of the repo go to a durable folder, never to the scratchpad.
