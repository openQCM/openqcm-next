...
class MultiscanProcess(multiprocessing.Process):
    ...
    def baseline_coeffs_Vmag(self):
        ...
        (poly_Vmag, coeff_Vmag) = self.baseline_correction(self.freq_all, Vmag_c, Constants.BASELINE_POLY_ORDER)
...
# =============================================================================
        ...
        # the impedance splines below index against the sample number, as the
        # magnitude one does inside resonance.spline_fit
        xrange = range(len(filtered_mag))
        
        # VER 0.1.5a_G_DEV filtering and interpolation of phase signal
        # filtering Savitzky Golay
        filtered_phase = resonance.savitzky_golay(phase, window_size = SG_window_size, order = Constants.SG_order)
        # interpolation
        s_phase = UnivariateSpline(xrange, filtered_phase, s = Spline_factor)
        xs_phase = np.linspace(0, len(filtered_phase) - 1, points)
        phase_result_fit = s_phase(xs_phase)
        ...
        # filtering Savitzky Golay
        Vmag_filt = resonance.savitzky_golay(Vmag_corr, window_size = SG_window_size, order = Constants.SG_order)
        # interpolation
        ...
        V_phase_filt = resonance.savitzky_golay(Vphase, window_size = SG_window_size, order = Constants.SG_order)
        # interpolation
        ...
        Vmag_raw_filt = resonance.savitzky_golay(Vmag, window_size = SG_window_size, order = Constants.SG_order)
        s_Vmag_raw = UnivariateSpline(xrange, Vmag_raw_filt, s = Constants.SPLINE_FACTOR_G)
        Vmag_raw_result_fit = s_Vmag_raw(xs_Vmag)

        # PARAMETERS FINDER
        # VER 0.1.4 chenge the bandwith threshold value to the constant value THRESHOLD_DB = 0.3
        # The band walk now lives in core/resonance.py, so the viewers can draw
        # exactly what is measured here instead of re-deriving it. On this branch
        # the magnitude-path result is kept only for the error flags: what gets
        # logged is the conductance-derived pair computed below.
        band = resonance.find_peak_and_band(freq_range, mag_result_fit, Constants.THRESHOLD_DB)

        # The two flags are sticky: they are cleared only once run() has pushed
        # them to parser6, so they report every overtone of the current cycle.
        if band.err_left:
...
# =============================================================================
        ...
        # 10 MHz get frequency sweep param
        if (peaks_mag_current[0] >9e+06 and peaks_mag_current[0]<11e+06):
            for i in range(peaks_mag_length):
                # get multiscan sweep paramters 
                (startF_temp, 
                 stopF_temp, 
                 SG_window_size_temp, 
                 spline_factor_temp) = self.getMultiscanParameters_10Mhz(peaks_mag_current, i)
        ...
        if (peaks_mag_current[0]> 4e+06 and peaks_mag_current[0]<6e+06):
            for i in range(peaks_mag_length):
                ...
                (startF_temp, 
                 stopF_temp, 
                 SG_window_size_temp, 
                 spline_factor_temp) = self.getMultiscanParameters_5Mhz(peaks_mag_current, i)
